...
function Spinner({ fullPage }) {
  const style = fullPage
    ...
    : { padding: "30px 0", display: "flex", alignItems: "center", justifyContent: "center" };
  return (
    <div style={style}>
      <div style={{ width: 36, height: 36, borderRadius: "50%", border: "3px solid rgba(255,255,255,0.1)", borderTop: "3px solid #1db954", animation: "spin 0.8s linear infinite" }} />
      <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
    </div>
  );
}

// ── FIX #5: Error Banner с кнопкой повтора ────────────────────────────────
function ErrorBanner({ message, onRetry }) {
  if (!message) return null;
  return (
    <div style={{ ...S.errorBox, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
      <span>⚠️ {message}</span>
      {onRetry && <button onClick={onRetry} style={{ ...S.btnSecondary, fontSize: 11, padding: "4px 10px", flexShrink: 0 }}>Повторить</button>}
    </div>
  );
}

// ── FIX #9: Page transition ────────────────────────────────────────────────
...
// ── TrackCard ──────────────────────────────────────────────────────────────
const TrackCard = memo(function TrackCard({ t, idx, list, onPlay, onToggleFav, fav, isPlaying, extraBadge, onAddToPlaylist }) {
  return (
    <div style={{ ...S.trackCard, background: isPlaying ? "rgba(29,185,84,0.1)" : S.trackCard.background, border: isPlaying ? "1px solid rgba(29,185,84,0.3)" : "1px solid transparent" }}>
      <div style={S.trackCover} onClick={() => onPlay(t, list, idx)}>
        {t.coverUrl ? <img src={t.coverUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt={t.title} loading="lazy" /> : "🎵"}
      </div>
...
// ── FIX #1 #2: PageCatalog вынесен из App, searchRef стабильный ───────────
const PAGE_SIZE = 40;

function PageCatalog({ ready, me, tracks, favorites, current, error, hasAccess, onPlay, onToggleFav, onStartTrial, onSubscribe, onBoot, onSearch, onClearSearch }) {
  ...
  // FIX #6: Пагинация
  const visibleTracks = useMemo(() => tracks.slice(0, PAGE_SIZE * (page + 1)), [tracks, page]);
  const hasMore = tracks.length > PAGE_SIZE * (page + 1);
        ...
        <div style={S.userCard}>
          ...
          {!hasAccess && (
            <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
              <button style={S.btnPrimary} onClick={onStartTrial}>Пробный</button>
              <button style={S.btnSecondary} onClick={onSubscribe}>Купить</button>
            </div>
          )}
        </div>
      )}

      {error && <ErrorBanner message={error} onRetry={onBoot} />}

      {/* FIX #3: Спиннер при загрузке */}
      {!ready && !error && <Spinner fullPage />}

      {!ready && error && (
        <div style={{ ...S.card, textAlign: "center" }}>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", marginBottom: 12 }}>Открой Mini App внутри Telegram</div>
          <button style={S.btnPrimary} onClick={onBoot}>Попробовать снова</button>
        </div>
      )}
      ...
      {ready && (
        <div>
          <div style={S.searchWrap}>
            {/* FIX #2: ref живёт в стабильном компоненте — фокус не теряется */}
            <input
              ref={searchRef}
              value={q}
              ...
              style={S.searchInput}
            />
            <button style={S.searchBtn} onClick={handleSearch}>Найти</button>
            {q && <button style={S.btnSecondary} onClick={handleClear}>✕</button>}
          </div>
            ...
            <>
              <div style={S.grid}>
                {visibleTracks.map((t, idx) => (
                  <TrackCard key={t.id} t={t} idx={idx} list={tracks} onPlay={onPlay} onToggleFav={onToggleFav} fav={isFav.has(t.id)} isPlaying={current?.id === t.id} />
                ))}
              </div>
              {hasMore && (
                <div style={{ textAlign: "center", padding: "16px 0" }}>
                  <button style={S.btnSecondary} onClick={() => setPage(p => p + 1)}>
                    Показать ещё ({tracks.length - visibleTracks.length})
                  </button>
                </div>
              )}
            </>
...
// ── PageFavorites ──────────────────────────────────────────────────────────
function PageFavorites({ favorites, current, onPlay, onToggleFav }) {
  const isFav = useMemo(() => new Set(favorites.map(t => t.id)), [favorites]);
  return (
    <div style={S.page}>
      ...
      {favorites.length === 0 ? (
        ...
        <div style={S.grid}>
          {favorites.map((t, idx) => (
            <TrackCard key={t.id} t={t} idx={idx} list={favorites} onPlay={onPlay} onToggleFav={onToggleFav} fav={isFav.has(t.id)} isPlaying={current?.id === t.id} />
          ))}
        </div>
      )}
    </div>
...
// ── FIX #7: PageHistory с кэшем ───────────────────────────────────────────
const historyCache = { data: null, top: null };

function PageHistory({ current, favorites, onPlay, onToggleFav }) {
  const isFav = useMemo(() => new Set(favorites.map(t => t.id)), [favorites]);
  ...
  useEffect(() => {
    ...
    Promise.all([api.history(), api.topTracks()])
      .then(([h, top]) => {
        historyCache.data = h.history || [];
        historyCache.top = top.topTracks || [];
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.grid}>
        {list.map((t, idx) => (
          <TrackCard key={t.id} t={t} idx={idx} list={list} onPlay={onPlay} onToggleFav={onToggleFav} fav={isFav.has(t.id)} isPlaying={current?.id === t.id} extraBadge={tab === "top" && t.userPlayCount ? `▶ ${t.userPlayCount}` : null} />
        ))}
      </div>
    </div>
  );
...
// ── PageArtistDetail ───────────────────────────────────────────────────────
function PageArtistDetail({ artistId, onBack, current, onPlay }) {
  ...
  useEffect(() => {
    Promise.all([
      api.artist(artistId),
      fetch(`${API_BASE}/artists/${artistId}/albums`, { headers: { Authorization: `Bearer ${getToken()}` } }).then(r => r.json())
    ])
  ...
  async function handleOpenAlbum(album) {
    setOpenAlbum(album);
    try {
      const res = await fetch(`${API_BASE}/albums/${album.id}`, { headers: { Authorization: `Bearer ${getToken()}` } }).then(r => r.json());
      ...
      <div>
        ...
        <div style={{ padding: "0 16px 20px", display: "flex", gap: 16, alignItems: "flex-end" }}>
          <div style={{ width: 110, height: 110, borderRadius: 12, overflow: "hidden", background: "rgba(255,255,255,0.07)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40 }}>
            {openAlbum.coverUrl ? <img src={openAlbum.coverUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="" /> : "💿"}
          </div>
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>{artist.name}{openAlbum.year ? ` · ${openAlbum.year}` : ""}</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)", marginTop: 2 }}>{albumTracks.length} {albumTracks.length === 1 ? "трек" : albumTracks.length < 5 ? "трека" : "треков"}</div>
          </div>
        </div>
        <div style={{ padding: "0 16px" }}>
          {albumTracks.length === 0 ? <div style={S.empty}>Треков в альбоме нет</div> : albumTracks.map((t, idx) => (
            <div key={t.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", cursor: "pointer", border: current?.id === t.id ? "1px solid rgba(29,185,84,0.4)" : "1px solid transparent" }} onClick={() => onPlay(t, albumTracks, idx)}>
              <div style={{ width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {current?.id === t.id ? <span style={{ color: "#1db954", fontSize: 16 }}>▶</span> : <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 13 }}>{idx + 1}</span>}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
  ...
  return (
    <div>
      ...
      <div style={S.page}>
        {artist.bio && <div style={{ ...S.card, marginBottom: 20 }}><div style={{ fontSize: 13, color: "rgba(255,255,255,0.6)", lineHeight: 1.6 }}>{artist.bio}</div></div>}
        {albums.length > 0 && (
          <div style={{ marginBottom: 24 }}>
            <div style={{ ...S.sectionTitle, marginBottom: 12 }}>Альбомы</div>
            <div style={{ display: "flex", gap: 12, overflowX: "auto", paddingBottom: 8 }}>
              {albums.map(album => (
                <div key={album.id} style={{ flexShrink: 0, width: 120, cursor: "pointer" }} onClick={() => handleOpenAlbum(album)}>
                  <div style={{ width: 120, height: 120, borderRadius: 10, overflow: "hidden", background: "rgba(255,255,255,0.07)", marginBottom: 6, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36 }}>
                  ...
                  <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 2 }}>
                    {album.year || ""}{album.year && album._count?.tracks ? " · " : ""}{album._count?.tracks ? `${album._count.tracks} тр.` : ""}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {artTracks.map((t, idx) => (
            <div key={t.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 12, padding: "10px 12px", cursor: "pointer", border: current?.id === t.id ? "1px solid rgba(29,185,84,0.4)" : "1px solid transparent" }} onClick={() => onPlay(t, artTracks, idx)}>
...
// ── FIX #7: PageArtists с кэшем ───────────────────────────────────────────
const artistsCache = { data: null };

function PageArtists({ current, onPlay }) {
  ...
  if (openId) return <PageArtistDetail artistId={openId} onBack={() => setOpenId(null)} current={current} onPlay={onPlay} />;
...
// ── PageProfile ────────────────────────────────────────────────────────────
function PageProfile({ me, hasAccess, onStartTrial, onSubscribe, onBoot }) {
  return (
    <div style={S.page}>
      ...
      <div style={S.card}>
        ...
        {!hasAccess && (
          <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
            <button style={S.btnPrimary} onClick={onStartTrial}>Пробный период</button>
            <button style={S.btnSecondary} onClick={onSubscribe}>Купить подписку</button>
          </div>
        )}
      </div>
      <div style={S.card}>
        ...
        <button style={{ ...S.btnSecondary, marginTop: 10 }} onClick={onBoot}>Обновить статус</button>
      </div>
    </div>
  );
}
...
// ── PageBillingReturn ──────────────────────────────────────────────────────
function PageBillingReturn({ onBoot }) {
  return (
    <div style={S.page}>
      <div style={S.card}>
        ...
        <button style={{ ...S.btnPrimary, marginTop: 12 }} onClick={onBoot}>Проверить доступ</button>
      </div>
    </div>
  );
}
...
function PageAdmin({ isAdmin }) {
  ...
  async function loadAdminAlbums() {
    try {
      const r = await fetch(`${API_BASE}/admin/albums`, { headers: { Authorization: `Bearer ${getToken()}` } }).then(res => res.json());
  ...
  async function deleteArtist(id) {
    setBusy(true);
    try {
      await fetch(`${API_BASE}/admin/artists/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  async function createAlbum() {
    ...
    try {
      await fetch(`${API_BASE}/admin/albums`, {
        method: "POST",
  ...
  async function uploadAlbumCover(id) {
    ...
    try {
      ...
      await fetch(`${API_BASE}/admin/albums/${id}/cover`, { method: "POST", headers: { Authorization: `Bearer ${getToken()}` }, body: fd });
  ...
  async function saveAlbumEdit(id) {
    ...
    try {
      await fetch(`${API_BASE}/admin/albums/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
  ...
  async function deleteAlbum(id) {
    setBusy(true);
    try {
      await fetch(`${API_BASE}/admin/albums/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  async function openAlbumDetail(id) {
    try {
      const r = await fetch(`${API_BASE}/albums/${id}`, { headers: { Authorization: `Bearer ${getToken()}` } }).then(res => res.json());
  ...
  async function addTrackToAlbum(albumId) {
    ...
    try {
      await fetch(`${API_BASE}/admin/albums/${albumId}/tracks`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${getToken()}` },
  ...
  async function removeTrackFromAlbum(albumId, trackId) {
    setBusy(true);
    try {
      await fetch(`${API_BASE}/admin/albums/${albumId}/tracks/${trackId}`, { method: "DELETE", headers: { Authorization: `Bearer ${getToken()}` } });
      await openAlbumDetail(albumId);
    } catch (e) { setAdminError(String(e.message || e)); } finally { setBusy(false); }
  }

  if (!isAdmin) return <div style={S.page}><div style={S.empty}>Только для администратора</div></div>;
  ...
  return (
    <div style={S.page}>
      ...
      {adminError && <ErrorBanner message={adminError} onRetry={() => setAdminError("")} />}
...
export default function App() {
  ...
  // FIX #5: Понятные сообщения об ошибках сети
  async function boot() {
    setError("");
    try {
      ...
      // Получаем высоту статус-бара от Telegram
  ...
  // FIX #1 #4: Компоненты вынесены, PagePlaylists убран полностью
  let page;
  if (route.startsWith("#/favorites")) {
    page = <PageFavorites favorites={favorites} current={current} onPlay={play} onToggleFav={toggleFav} />;
  } else if (route.startsWith("#/history")) {
    page = <PageHistory current={current} favorites={favorites} onPlay={play} onToggleFav={toggleFav} />;
  } else if (route.startsWith("#/artists")) {
    page = <PageArtists current={current} onPlay={play} />;
  } else if (route.startsWith("#/profile")) {
    page = <PageProfile me={me} hasAccess={hasAccess} onStartTrial={startTrial} onSubscribe={subscribe} onBoot={boot} />;
  } else if (route.startsWith("#/billing/return")) {
    page = <PageBillingReturn onBoot={boot} />;
  } else if (route.startsWith("#/admin")) {
    page = <PageAdmin isAdmin={isAdmin} />;
  } else {
    page = (
      <PageCatalog
        ready={ready} me={me} tracks={tracks} favorites={favorites}
        current={current} error={error} hasAccess={hasAccess}
        onPlay={play} onToggleFav={toggleFav}
        onStartTrial={startTrial} onSubscribe={subscribe}
        onBoot={boot} onSearch={handleSearch} onClearSearch={handleClearSearch}
      />
    );
  }
  ...
  return (
    <div style={S.app}>
      <div style={{ ...S.header, paddingTop: safeTop ? `${safeTop + 12}px` : "12px" }}>
        <div>
      ...
      {/* FIX #9: Анимация при переходе между вкладками */}
